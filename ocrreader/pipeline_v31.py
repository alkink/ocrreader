...
# ── v31 rescue constants ──────────────────────────────────────────────────────

# Engine: patterns that match real engine codes vs OCR label artifacts
# Label artifacts look like D3TICARIADI, Y2TESCILSIRANO (letter + digit + 3+ letters)
_ENGINE_LABEL_ARTIFACT_RE = re.compile(r"^[A-Z]{1,2}\d[A-Z]{3,}")
# Real engine codes: 1-3 letters then 5-12 alphanums; K9K* Renault; long pure-digit (Mercedes)
_ENGINE_CODE_RE = re.compile(r"^(?:[A-Z]{1,3}\d{5,12}|K9K[A-Z0-9]{5,9})$")
_ENGINE_YEAR_RE = re.compile(r"^(?:19|20)\d{2}$")
_ENGINE_LABEL_WORDS = frozenset({
    "OTOMOBIL", "KAMYONET", "TRANSIT", "TOURNEO", "RENAULT", "VOLKSWAGEN",
    "FORD", "BEYAZ", "SINIFI", "MARKASI", "MOTORNO", "SASENO", "ESASENO",
    "BELGE", "SERI", "TESCIL", "MODELYILI", "TIPI", "PLAKA",
    "SINIFT", "AGIRLIGI", "KOLTUK", "YOLCUSAYISI", "TRANSIT/TOURNEO",
    "MERCEDES", "MAZDA", "OPEL", "KIA", "HONDA", "HYUNDAI", "PEUGEOT",
})

# Serial: patterns for merged "Seri[code]NNNNNN" tokens
_SERIAL_MERGED_RE = re.compile(
    r"(?:Seri?|Sen|Sec|Sc|Se)[A-Za-z]{0,6}(\d{4,8})",
    re.IGNORECASE,
)
_SERIAL_SUFFIX_RE = re.compile(r"[A-Z]{1,4}(\d{4,8})$")
_SERIAL_PURE_RE = re.compile(r"^\d{5,8}$")  # min 5 digits: avoids 4-digit page numbers
_SERIAL_YEAR_RE = re.compile(r"^(?:19|20)\d{2}$")


def _rescue_engine_no(
    words: list[OCRWord],
    doc_shape: tuple[int, int, int],
) -> str | None:
    """
    v31: Last-resort engine_no extraction.

    When the anchor-based extraction returns empty, scan page words in the left
    column using pattern matching. Filters out OCR label artefacts.
    Only call when ``fields['engine_no']`` is empty.
    """
    h, w = doc_shape[:2]
    if h == 0 or w == 0:
        return None
    ...
    for word in words:
        ...
        # Engine label is in left half; avoid header (top 35%) and footer (bottom 20%)
        ...
        if len(clean) < 5 or len(clean) > 15:
            continue
        if len(clean) == 17:  # full VIN → chassis, not engine
            continue
        if _ENGINE_YEAR_RE.match(clean):
            continue
        if clean in _ENGINE_LABEL_WORDS:
            continue
        if clean.isalpha():
            continue
        if _ENGINE_LABEL_ARTIFACT_RE.match(clean):  # D3TICARIADI, Y2TESCIL…
            continue
        if not _ENGINE_CODE_RE.match(clean):
            continue

        candidates.append((cy, clean))

    if not candidates:
        return None

    # Prefer candidates in the engine section (lower third of left column)
    lower = [(cy, c) for cy, c in candidates if cy >= 0.45]
    pool = lower if lower else candidates

    def _strength(item: tuple[float, str]) -> int:
        _, c = item
        if re.match(r"^K9K", c):
            return 3
        if re.match(r"^[A-Z]{2}\d{5,7}$", c):  # EG37672, GK88874
            return 2
        if re.match(r"^[A-Z]{3}\d{5,9}$", c):  # DCX033089, CAY C...
            return 1
        return 0

    pool.sort(key=lambda x: (-_strength(x), x[0]))
    return pool[0][1]
...
def _rescue_serial_no(
    ...
    """
    v31: Last-resort serial_no extraction.

    PaddleOCR often merges the 'Seri[code]NNNNNN' label+value into one token.
    This function:
      1. Looks for that merged pattern in the serial search region.
      2. Falls back to a standalone digit sequence if present.
    Only call when ``fields['serial_no']`` is empty.
    """
    h, w = doc_shape[:2]
    if h == 0 or w == 0:
        return None

    # Default serial search region: right half, bottom 20% of form
    ...
    # Pass 1: merged Seri* tokens  → extract trailing digit group
    for text in region_texts:
        m = _SERIAL_MERGED_RE.search(text)
        if m and len(m.group(1)) >= 4:
            return m.group(1)

    # Pass 2: "SBEN484693", "SenCHN664423" style → 1-4 alpha suffix + digits
    for text in region_texts:
        m = _SERIAL_SUFFIX_RE.search(text.upper())
        if m and len(m.group(1)) >= 4 and not _SERIAL_YEAR_RE.match(m.group(1)):
            return m.group(1)

    # Pass 3: standalone digit token in the region
    for text in region_texts:
        clean = re.sub(r"[^0-9]", "", text)
        if _SERIAL_PURE_RE.match(clean) and not _SERIAL_YEAR_RE.match(clean):
            return clean

    return None


def _apply_v31_rescues(
    fields: dict[str, dict[str, object]],
    words: list[OCRWord],
    doc_shape: tuple[int, int, int],
    serial_search_region_norm: list[float] | None = None,
) -> None:
    """
    v31: Apply engine_no and serial_no rescues when primary extraction failed.
    Called BEFORE postprocess_fields so the values pass through validation.
    """
    # --- engine_no rescue ---
    if _is_empty_field_entry(fields.get("engine_no")):
        recovered = _rescue_engine_no(words, doc_shape)
        if recovered:
            entry = dict(fields.get("engine_no") or {})
            entry["value"] = recovered
            entry["raw"] = recovered
            entry["method"] = "v31_engine_page_rescue"
            entry["confidence_score"] = 55
            entry["low_confidence"] = False
            fields["engine_no"] = entry

    # --- serial_no rescue ---
    if _is_empty_field_entry(fields.get("serial_no")):
        recovered = _rescue_serial_no(words, doc_shape, serial_search_region_norm)
        if recovered:
            entry = dict(fields.get("serial_no") or {})
            entry["value"] = recovered
            entry["raw"] = recovered
            entry["method"] = "v31_serial_page_rescue"
...
class RuhsatOcrPipeline:
    ...
    def process_path(
        ...
        # v31: pattern-based rescues for engine_no and serial_no
        # NOTE: use only unified rescue path below; legacy rescue helpers above
        # are intentionally not invoked to avoid aggressive FP regressions.
        serial_region = None
        if "serial_label" in self.config.anchors:
            serial_region = self.config.anchors["serial_label"].search_region_norm
